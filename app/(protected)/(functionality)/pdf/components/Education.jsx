            ...
            {educationData.map((item, index) => (
                <div key={item.id} className="border rounded p-4 mb-4 last:mb-0 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {/* Input fields and display logic for Education items */}
                        {Object.keys(item)
                            .filter((key) => key !== 'id')
                            .map((field) => (
                                <div key={field} className="mb-2">
                                    ...
                                    {field.includes('date')
                                        ? isEditing(item.id) && <Calendar value={item[field] ? new Date(item[field]) : null} onChange={(e) => handleDateChange(index, field, e)} dateFormat="yy-mm-dd" className="w-full" />
